...
describe("App: Catalog tab", () => {
  it("TABS includes \"Catalog\" and \"Config\"", async () => {
    ...
    unmount(); // unmount App so the background detect does not disturb the next test
  });
  it("empty start → the Catalog tab is active (the plugin list is visible)", async () => {
    const { lastFrame, unmount } = render(<App />);
